...
describe('TripForm Accessibility & UI rendering', () => {
  it('should render form fields with accessible names and labels', () => {
    ...
    // Destination Input
    const destinationInput = screen.getByLabelText(/Where are you traveling to\?/i);
    ...
    // Submit button
    const submitBtn = screen.getByRole('button', { name: /Generate Immersive Guide/i });
  ...
  it('should display validation error if submitted with destination but no interests selected', async () => {
    ...
    const alert = await screen.findByRole('alert');
    expect(alert).toBeInTheDocument();
    expect(alert.textContent).toContain('Please select at least one cultural interest.');
    expect(handleSubmitMock).not.toHaveBeenCalled();
  });
  ...
  it('should call onSubmit with formatted payload when valid values are entered', () => {
    ...
    const historyBtn = screen.getByRole('button', { name: /History & Heritage/i });
  ...
  it('should disable form submit button when isLoading is true', () => {
    ...
    const submitBtn = screen.getByRole('button', { name: /Generating Culture Trail/i });
    expect(submitBtn).toBeInTheDocument();
